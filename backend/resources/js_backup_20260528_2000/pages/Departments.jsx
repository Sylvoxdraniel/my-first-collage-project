...
export default function Departments() {
  ...
  const activeDept = searchParams.get('dept') || 'computer';
  ...
  const dept = departmentsList[activeDept] || departmentsList.computer;
    ...
    <div className="max-w-7xl mx-auto px-6 py-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 bg-white border border-slate-200 p-2 lg:p-4 rounded-2xl shadow-sm h-fit flex lg:flex-col gap-1 overflow-x-auto lg:overflow-x-visible whitespace-nowrap lg:whitespace-normal scrollbar-none">
          {Object.keys(departmentsList).map((key) => (
            <button
              key={key}
              onClick={() => handleDeptChange(key)}
              className={`px-4 py-2.5 rounded-lg text-xs font-bold transition-all shrink-0 text-left lg:w-full capitalize ${
                activeDept === key
                  ? 'bg-blue-900 text-white shadow-md'
                  : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'
              }`}
            >
              {key === 'arts' || key === 'computer' || key === 'education'
        ...
        {/* Content Box */}
        <div className="lg:col-span-3 bg-white border border-slate-200 p-4 sm:p-6 lg:p-8 rounded-3xl shadow-sm min-h-[500px] space-y-8">
